...
int main() {
    ...
    size_t node_count = (1 << (DEPTH + 1)) - 1;
    lab::print_qor("Mapping Tree", node_count * sizeof(Node), node_count);

    {
        lab::Benchmark b("Area-Optimized Mapping");
        mapper.map_area(root);
        std::cout << "[METRICS] Optimized Area: " << root->best_area << std::endl;
    }

    {
        lab::Benchmark b("Delay-Optimized Mapping");
        mapper.map_delay(root);
        std::cout << "[METRICS] Optimized Delay: " << root->best_delay << std::endl;
    }
